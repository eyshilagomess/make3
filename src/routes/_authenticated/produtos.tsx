...
// Cabeçalho de referência (usado só quando o usuário NÃO envia o modelo oficial).
const TIKTOK_TEMPLATE_HEADER: string[][] = tiktokTemplate as string[][];

// O modelo do TikTok muda de colunas conforme a categoria/país, então o
// mapeamento é feito SEMPRE pelos nomes técnicos da 1ª linha do arquivo.
type TiktokCols = Record<string, number>;
function mapTiktokCols(keys: any[]): TiktokCols {
  const m: TiktokCols = {};
  keys.forEach((k, i) => {
...
function tiktokProductIssues(p: any, opts: { category: string }) {
  const miss: string[] = [];
  if (!opts.category) miss.push("categoria");
  if (!String(p.name ?? "").trim()) miss.push("nome");
  if (!String(p.description ?? "").trim()) miss.push("descrição");
  if (!String(p.photo_url ?? "").trim()) miss.push("foto principal");
  if (!(Number(p.price_tiktok) > 0)) miss.push("preço TikTok");
  return miss;
}

function buildTiktokRows(products: any[], opts: { category: string; brand: string }, cols: TiktokCols, colCount: number) {
  const rows: any[][] = [];
  const skipped: { name: string; miss: string[] }[] = [];

  for (const p of products) {
    if (p.status && p.status !== "ativo") continue;

    const miss = tiktokProductIssues(p, opts);
    if (miss.length > 0) {
      skipped.push({ name: String(p.name ?? "(sem nome)"), miss });
      continue;
    }

    const make = (o: { price: any; stock: any; sku: any; variation?: string }) => {
      const r: any[] = new Array(colCount).fill("");
      const set = (key: string, value: any) => {
        const i = cols[key];
        if (i != null && value !== undefined && value !== null && value !== "") r[i] = value;
      };
      set("category", opts.category);
      set("brand", opts.brand);
      set("product_name", String(p.name ?? "").slice(0, 255));
      set("product_description", String(p.description || p.name || "").slice(0, 10000));
      set("main_image", p.photo_url);
      if (o.variation) {
        set("property_name_1", "Cor");
        set("property_value_1", String(o.variation).slice(0, 50));
        set("property_1_image", p.photo_url);
      }
      set("parcel_weight", Number(p.weight_g ?? 200));
      set("parcel_length", Number(p.length_cm ?? 20));
      set("parcel_width", Number(p.width_cm ?? 15));
      set("parcel_height", Number(p.height_cm ?? 10));
      set("price", Number(Number(o.price).toFixed(2)));
      set("quantity", Math.max(0, Number(o.stock ?? 0)));
      set("seller_sku", o.sku);
      return r;
    };
...
async function exportTiktokFromTemplate(products: any[], file: File, opts: { category: string; brand: string }, perFile = 0) {
  const buf = await file.arrayBuffer();
  const probe = XLSX.read(buf, { type: "array" });

  let target: string | undefined;
  for (const name of probe.SheetNames) {
    const first = XLSX.utils.sheet_to_json<any[]>(probe.Sheets[name], { header: 1, blankrows: true })[0] ?? [];
    if (first.some((c) => String(c ?? "").trim() === "product_name")) { target = name; break; }
  }
  if (!target) throw new Error("Não encontrei a aba de produtos no modelo do TikTok enviado.");

  const baseAoa = XLSX.utils.sheet_to_json<any[]>(probe.Sheets[target], { header: 1, blankrows: true, defval: "" });
  const keys = baseAoa[0] ?? [];
  const cols = mapTiktokCols(keys);
  const colCount = Math.max(keys.length, ...baseAoa.slice(0, 6).map((r) => (r ?? []).length));
  const headerRows = baseAoa.slice(0, 6).map((r) => {
    const row = [...(r ?? [])];
  ...
  const batches = chunk(products, perFile);
  let rowCount = 0;
  const skippedAll: { name: string; miss: string[] }[] = [];
  let files = 0;

  for (let i = 0; i < batches.length; i++) {
    const { rows, skipped } = buildTiktokRows(batches[i], opts, cols, colCount);
    ...
    XLSX.writeFile(wb, tiktokFileName(files, batches.length), { bookType: "xlsx", compression: true });
    if (i < batches.length - 1) await wait(700);
  }
  return { rows: rowCount, files, skipped: skippedAll };
}

async function exportTiktokGeneric(products: any[], opts: { category: string; brand: string }, perFile = 0) {
  const keys = TIKTOK_TEMPLATE_HEADER[0];
  const cols = mapTiktokCols(keys);
  const batches = chunk(products, perFile);
  let rowCount = 0;
  let files = 0;
  const skippedAll: { name: string; miss: string[] }[] = [];
  for (let i = 0; i < batches.length; i++) {
    const { rows, skipped } = buildTiktokRows(batches[i], opts, cols, keys.length);
    skippedAll.push(...skipped);
    if (rows.length === 0) continue;
    rowCount += rows.length;
    files += 1;
